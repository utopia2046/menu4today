...
function getDishesList(sourceList, filters = []) {
  ...
  sourceList.forEach((dish) => {
    ...
    if (filters && filters.length > 0) {
      let testResults = _.map(
        filters,
        (filter) => {
          return testDishWithFilter(dish, filter);
        },
        this
      );
      let result = _.reduce(
...
function testDishWithFilter(dish, filter) {
  switch (filter.by) {
    case consts.filterBy.title:
      return (
        (filter.op === consts.filterOp.contains &&
          dish.name.includes(filter.value)) ||
        (filter.op === consts.filterOp.notContain &&
          !dish.name.includes(filter.value))
      );
    case consts.filterBy.ingredient:
      return (
        (filter.op === consts.filterOp.contains &&
          hasIngredInDish(dish, filter.value)) ||
        (filter.op === consts.filterOp.notContain &&
          !hasIngredInDish(dish, filter.value))
      );
    case consts.filterBy.category:
      return (
        (filter.op === consts.filterOp.contains &&
          dish.category.includes(filter.value)) ||
        (filter.op === consts.filterOp.notContain &&
          !dish.category.includes(filter.value))
      );
    case consts.filterBy.tag:
      return (
        (filter.op === consts.filterOp.contains &&
          _.contains(dish.tags, filter.value)) ||
        (filter.op === consts.filterOp.notContain &&
          !_.contains(dish.tags, filter.value))
      );
    case consts.filterBy.utensil:
      return (
        (filter.op === consts.filterOp.contains &&
          dish.utensil.includes(filter.value)) ||
        (filter.op === consts.filterOp.notContain &&
          !dish.utensil.includes(filter.value))
      );
...
function getMeal(peopleSize, mealType, filters) {
  if (mealType === consts.mealType.breakfast) {
    ...
    if (
      peopleSize === consts.peopleSize.single &&
      mealType === consts.mealType.lunch
    ) {
      const soup1 = getRandomElementsFromArray(getDishesList(soup), 1);
      return getMealMenu(mealType, _.union(dishes, soup1));
    }
